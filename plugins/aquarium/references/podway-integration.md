...
- ZCode todo lists and Podway goals have separate creation rules and lifecycles. Create a ZCode todo list only when explicitly requested under its current tool contract; a Podway goal does not authorize one. When both are authorized, keep their scope consistent with the user objective and roadmap without copying the complete criteria or status history. A bounded Podway session may cover only part of the ZCode todo list.
...
Follow the pinned skill's context recovery guidance when resuming an authorized Podway workflow. Read its `references/goal.md` when a ZCode todo list accompanies that workflow. Context restoration or automatic continuation of the same workflow retains its established authorization and boundaries; it does not repeat activation or the entry archive advisory.
...
A terminal session, null guidance, or an `achieved` assessment does not independently prove the ZCode todo-list objective complete. Verify the actual requirements and external results, and continue any remaining authorized work. A Podway blocker does not automatically block a ZCode todo list: follow the current host todo-list contract, including any recurrence threshold, and continue useful independent work. Never change Podway lifecycle state to synchronize goal statuses.
