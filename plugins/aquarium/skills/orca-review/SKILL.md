...
`workspace` and `dirty` remain outside this workflow. Report the unsupported Orca scope and ask for an explicitly selected supported target or review route; Independent Review is available through `/aquarium:independent-review` on explicit selection, `workspace` and `dirty` require an immutable capture no backend here provides, and no route is an automatic fallback. Never stage paths or reinterpret state merely to manufacture an Orca Review target.
