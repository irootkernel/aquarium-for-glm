---
name: task-handler
description: "Strengthen or resume the procedure around one named roadmap task goal through planning, implementation, verification, refinement, documentation, review, and closeout. Use when the user explicitly invokes /aquarium:task-handler with a roadmap path and exactly one task ID; never invoke it implicitly."
---
...
Otherwise read the Podway contract, use one `aquarium-task-v2` session for this canonical task after plan approval, coordinate with an explicitly authorized ZCode todo list under the shared Podway contract, and record each verified phase handoff at the matching node. In `plan-only`, create neither goal nor session; in `plan-handoff`, create the session only after approval, create a ZCode todo list only on explicit request, and stop at `implement` with the verified plan artifact. Do not let either goal mechanism or runtime evidence replace roadmap authority.
...
Create a ZCode todo list only after plan approval and an explicit user request under its tool contract. When authorized, inspect the current goal, continue it when it represents the same task, create one containing the task ID and evidence boundary when none exists, and stop rather than replace a different unfinished goal. Otherwise proceed without a ZCode todo list. Omit a token budget unless the user explicitly supplied one.

When a ZCode todo list is authorized, keep it active through every phase. Mark it complete only after `/aquarium:task-close` succeeds and no required task work or authorized lifecycle action remains. Mark the goal blocked only when the host's goal tool defines a blocked state and its own repeated-blocker rule is met by the same unresolved external blocker persisting across consecutive goal turns with no authorized action remaining; otherwise keep it active and report the exact gap.
