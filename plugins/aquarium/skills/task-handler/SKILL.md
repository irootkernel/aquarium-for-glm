...
Otherwise read the contract, own one `aquarium-task-v2` session for this canonical task after plan approval, mirror its current goal in the ZCode todo list, and record each verified phase handoff at the matching node. In `plan-only`, create neither goal nor session; in `plan-handoff`, create them only after approval and stop at `implement` with the verified plan artifact. Do not let either goal mechanism replace roadmap authority.
