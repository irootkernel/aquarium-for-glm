...
`sync.py` copies the upstream plugin, applies literal substitutions, reworks whole functions of the bundled inspection script through name-anchored surgery, applies overrides, drops the Codex sidecar directories, and derives the manifest. It refuses to run against an empty submodule, refuses to run when upstream grows a directory the transformation does not handle, and fails if host-specific text survives.
...
Upstream diagnoses Ouroboros by asking Codex — `ooo codex doctor` for its integration artifacts and `codex mcp get <server> --json` for the Ouroboros, Mulgae, and Gaori registrations, probing a user-global, an isolated project-local, and an effective view with repository-bound arguments, timeout floors, and `required` flags parsed from Codex output. ZCode has neither probe, the `ooo zcode` group ships no doctor command, and registrations here carry no per-server timeout surface, so the surgical replacements classify every MCP registration — `ouroboros`, `mulgae`, and `gaori` — from the host's config files: the user-level `~/.zcode/cli/config.json`, the project-level `.zcode/config.json` override, and the effective scope the project entry wins. The Ouroboros entry doubles as the host-integration signal, because on this host the integration is exactly that entry plus the user-scoped Ouroboros skills. Runtime health comes from `ooo mcp doctor --json` with the `mcp_import` check exempted: the MCP 2 server registered in config launches as a separate process while the CLI environment keeps MCP 1.x, so that check fails on a correctly configured machine.
