...
Merge the MCP entry into the user-level `mcp.servers` object in `~/.zcode/cli/config.json`, preserving unrelated entries, or let the approved `ooo setup --runtime zcode` write it and verify the result against this shape:
